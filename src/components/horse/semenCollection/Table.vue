<template>
  <div>
    <v-data-table :headers="headers" :items="filteredSemenCollections()" :loading="loading"
                  loading-text="Bezig met laden..." class="ma-5">
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-spacer/>
          <v-btn color="primary" dark @click="openDialog('create')" class="mr-2 d-print-none">Sperma lot toevoegen
          </v-btn>
          <v-btn color="primary" dark @click="filterDialog = true" class="d-print-none">
      ...
      <template v-slot:item="props">
        <tr>
          ...
          <td>
            <v-icon v-if="props.item.color" small class="mr-2" :class="strawColor(props.item.color)">
              mdi-checkbox-blank-circle
            </v-icon>
            {{ props.item.location.container.name }} - Koker {{ props.item.location.tube }} -
          ...
          <td class="d-print-none text-right">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon dark color="primary" @click="openDialog('create', props.item)" v-on="on" class="mr-2">
                  mdi-pencil
                </v-icon>
              </template>
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon dark color="primary" @click="openDialog('edit', props.item)" v-on="on" class="mr-2">mdi-plus
                </v-icon>
              </template>
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-icon dark color="primary" @click="openDialog('detail', props.item)" v-on="on" class="mr-2">
                  mdi-magnify
                </v-icon>
              </template>
    ...
    <create-dialog
        ...
        :createDialog="dialogs.create"
        @created-semen-collection="addSemenCollection"
        @update-semen-collection="updateSemenCollection"
        @close-dialog="dialogs.create = false"
    />
    <edit-dialog
    ...
    <detail-dialog
        ...
        @update-semen-collection="updateSemenCollection"
        :semenCollection="semenCollection"
        :detailDialog="dialogs.detail"
        @close-dialog="dialogs.detail = false"
    />
    <delete-dialog
        v-if="semenCollection"
        @update-semen-collection="updateSemenCollection"
        @delete-semen-collection="deleteSemenCollection"
        :semenCollection="semenCollection"
      ...
      <v-card>
        <v-card-text class="pt-5">
          <v-row dense>
            ...
            <v-col cols="12">
              <v-select clearable v-model="filters.type.value" outlined label="Filter op type lot"
                        :items="filters.type.options" hide-details/>
            </v-col>
            <v-col cols="12">
              <v-select clearable v-model="filters.modificationType.value" outlined
                        label="Filter op type stockwijzigingen" :items="filters.modificationType.options" hide-details/>
            </v-col>
          </v-row>
        </v-card-text>
        <v-card-actions>
